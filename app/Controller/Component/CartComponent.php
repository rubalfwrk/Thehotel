...
//////////////////////////////////////////////////
    ...
    public function add($id, $quantity = 1, $productmodId = null) {
        ...
        $data['price'] = $product['Product']['price'];
        $data['quantity'] = $quantity;
        $data['subtotal'] = sprintf('%01.3f', ($product['Product']['price'] * $quantity)+$product['Product']['box']);
        $data['totalweight'] = sprintf('%01.2f', $product['Product']['weight'] * $quantity);
        $data['Product'] = $product['Product'];
        $data['res_id'] = $product['Product']['res_id'];
        ...
        $cartdata['Cart']['price'] = $product['Product']['price'];
        $cartdata['Cart']['resid'] = $product['Product']['res_id'];
        $cartdata['Cart']['subtotal'] = sprintf('%01.3f', ($product['Product']['price'] * $quantity)+$product['Product']['box']);
		
		$this->Session->write('Shop.Order.total', sprintf('%01.3f', ($product['Product']['price'] * $quantity)+$product['Product']['box']));
...
//////////////////////////////////////////////////
    ...
    public function appcart($uid, $sid) {
        ...
        for ($i = 0; $i < $cnt; $i++) {
            ...
            $shop[$i]['Cart']['image'] = FULL_BASE_URL . "/dropin/files/product/" . $shop[$i]['Cart']['image'];
        }
