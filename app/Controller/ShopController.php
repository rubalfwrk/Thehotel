...
//////////////////////////////////////////////////
    ...
    public function beforeFilter() {
        parent::beforeFilter();
        $this->disableCache();
        $this->Auth->allow('api_allcountry', 'api_displayreviews', 'api_review', 'newsletter', 'api_webtime', 
                'api_CancelOrder', 'api_tablehistry', 'api_tablecancelorder', 'api_walletpayment',
                'api_creditcard','api_creditcardsucess','creditsuccess','creditsuccesstable','api_creditcardtable','ztime');
...
//////////////////////////////////////////////////
    ...
    public function reviewgetcartitem() {
        ...
        if (empty($cart['Order']['tablecalc'])) {
            ...
            $this->Session->write('Shop.Order.total', sprintf('%01.3f',$mytotl['Order']['total']));
...
//Merchant Details
                        ...
                        $ReturnURL = "http://rajdeep.crystalbiltech.com/dropin/shop/success";  //After Payment success, customer will be redirected to this url
                        $PostURL = "http://rajdeep.crystalbiltech.com/dropin/shop/review";  //After Payment success, Payment Data's will be posted to this url
//Product Details
...
//die();
                ...
                if ($pst_Data['payment_method'] == 'paypal') {
                    ...
                    <input type=\"hidden\" name=\"return\" value=\"http://rajdeep.crystalbiltech.com/dropin/shop/success\">
                    <input type=\"hidden\" name=\"notify_url\" value=\"http://rajdeep.crystalbiltech.com/dropin/shop/ipn\"> 
                    </form>";
//                    exit;
...
//               $cnt = count($data);
//                for ($i = 0; $i < $cnt; $i++) {                   
//                        $data[$i]['Restaurant']['logo'] = FULL_BASE_URL . $this->webroot . "files/product/" . $data[$i]['Restaurant']['logo'];                   
//                }
            $response['error'] = "0";
            $response['data'] = $data;
        } else {
 ...
 public function api_walletpaymentdinein() {
        ...
        if ($redata) {
           ...
           $order = array();
            ...
            $order['TableReservation']['first_name'] = $redata->address->billing->Table->fname;
            $order['TableReservation']['last_name'] = $redata->address->billing->Table->lname;
...
//Merchant Details
        ...
        $MerchantID = "13014";  //Merchant ID Provided by Tap
        $UserName = "test";   //Merchant UserName Provided by Tap
        $ReturnURL = "http://rajdeep.crystalbiltech.com/dropin/users/myaccount";  //After Payment success, customer will be redirected to this url
        $PostURL = "http://rajdeep.crystalbiltech.com/dropin/shop/ccwallet";  //After Payment success, Payment Data's will be posted to this url
//Product Details
        $CurrencyCode = "KWD";  //Order Currency Code
        $Total = $Mytotal; //$order['Order']['total']			//Total Order Amount
//Generating the Hash string
...
//                    $order['Order']['transaction'] = $authorizeNet[6];
    ...
    public function api_creditcardtable() {
       ...
       if ($redata) {
            ...
            $save = $this->TableReservation->saveAll($order, array('validate' => 'first'));
            $order_id = $this->TableReservation->getInsertID();
            $this->loadModel('PaymentHistory');  
            $this->request->data['PaymentHistory']['table_order_id']=$order_id;
              if ($save) {               
...
//                    ->send();
            ...
            if ($redata->payment->mode == 'creditcard') {
               ...
               // $save = $this->Order->saveAll($order, array('validate' => 'first'));
                    ...
                    $MerchantID = "13014";  //Merchant ID Provided by Tap
                    $UserName = "test";   //Merchant UserName Provided by Tap
                    $ReturnURL = "http://rajdeep.crystalbiltech.com/dropin/shop/creditsuccess";  //After Payment success, customer will be redirected to this url
                    $PostURL = "http://rajdeep.crystalbiltech.com/dropin/api/shop/creditcardsucess";  //After Payment success, Payment Data's will be posted to this url
                    $CurrencyCode = "KWD";  //Order Currency Code
                    $Total = $Mytotal; //$order['Order']['total']			//Total Order Amount
                    $APIKey = "tap1234";  //Merchant API Key Provided by Tap
                    $str = 'X_MerchantID' . $MerchantID . 'X_UserName' . $UserName . 'X_ReferenceID' . $ref . 'X_Mobile' . $Mobile . 'X_CurrencyCode' . $CurrencyCode . 'X_Total' . $Total . '';
