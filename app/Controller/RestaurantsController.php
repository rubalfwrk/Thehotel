...
class RestaurantsController extends AppController {
    ...
    public function beforeFilter() {
        ...
        $this->Auth->allow(array('api_restaurantslist', 'api_getresmenu',
            ...
            'api_advancepayment','api_promocode','api_deals','api_getalltype','api_restaurantslistall',
            'api_restaurantslistallwork','api_faq'));
    }
     ...
     * @return void
     */
    public function detail($id = null,$catname = null) {
        Configure::write("debug", 0);
		 
	$restDetail = $this->Restaurant->find('all',array('conditions'=>array('Restaurant.id'=>$id)));
       // print_r($restDetail);exit;
        $this->set('restdetail',$restDetail);  
        
        $latestDeals = $this->Restaurant->find('all', array(
            'limit' => 6,
            'order' => 'Restaurant.id DESC', 
           
        ));
        $this->set('latestdeals',$latestDeals); 
        $this->loadModel('Cat');
        $bread = $this->Cat->find('all',array('conditions'=>array('Cat.rest_type'=>$catname)));  
       
        //print_r($cb);exit;
       // print_r($bread);exit;
        $this->set('brndcrumb',$bread);
        
         $url = Router::url(
            ['controller' => 'restaurants', 'action' => 'detail', $id],
            true
        );
        
        if($url){
        $this->Restaurant->updateAll(array('Restaurant.hits' =>'Restaurant.hits + 1','Restaurant.modified'=>'now()'), array('Restaurant.id' => $id));
        } 
    }
...
public function categories($id = null){
       configure::write('debug', 0);
        ...
        $this->loadModel('Cat');
        $catname = $this->Cat->find('first',array('conditions'=>array('Cat.id'=>$id)));
        $this->set('catname',$catname);
        $this->loadModel('User');
        $this->Restaurant->recursive = 1;     
        $res = $this->Restaurant->find('all',array('conditions'=>array('AND'=>array('Restaurant.catid'=>$id,'Restaurant.status'=>1))));
        $this->set('restaurant' , $res);
        $uid= $this->Auth->user('id');
        $favs = $this->Favrest->find('all',array('conditions'=>array('Favrest.user_id'=>$uid)));
        foreach($favs as $fav){
           $favIds[] =  $fav['Favrest']['res_id']; 
        }
       // print_r($favIds);exit;
       $this->set('fav',$favIds);  
        $latestDeal = $this->Restaurant->find('all', array( 
            'limit' => 6,
            'order' => 'Restaurant.id DESC', 
           'conditions'=>array('Restaurant.status'=>1)
        ));
        $this->set('latestdeal',$latestDeal);
        //$this->Restaurant->find('all',array('conditions'=>array('Restaurant.'))
    }
    ...
    public function categories($id = null){
       configure::write('debug', 0);
        ...
        $this->loadModel('User');
        $this->Restaurant->recursive = 1;     
        $res = $this->Restaurant->find('all',array('conditions'=>array('Restaurant.catid'=>$id)));
        $this->set('restaurant' , $res);
        $uid= $this->Auth->user('id');
        $favs = $this->Favrest->find('all',array('conditions'=>array('Favrest.user_id'=>$uid)));
        foreach($favs as $fav){
    ...
    public function likeit() {
        ...
        if (!empty($arr)) {
            ...
            if ($data > 0) {
               ...
               // echo "ajay";
                $arr1 = array("res_id" => $res_id, "user_id" => $user_id);
               // print_r($arr1);exit;
                $response['data'] = $this->Favrest->save($arr1);
                $response['error'] = "1";
            }
        } else {
            $response['error'] = 1;
...
//                $data = $this->Restaurant->find('all', array('conditions' => array('AND' => $conditions)));
    ...
    public function delete($id = null) {
        ...
        if ($this->Restaurant->delete()) {
            echo $id;
            return $this->flash(__('The Deal has been deleted.'), array('action' => 'index'));
        } else {
            return $this->flash(__('The Deal could not be deleted. Please, try again.'), array('action' => 'index'));
        }
    }
...
//     $this->set(compact('users')); 
    ...
    /*********** autocomplete functionality by rahul**********/
...
//            pr($this->request->data);exit;    
            ...
            $marker = $this->request->data['Restaurant']['marker'];
            $uploadFolder = "restaurants";
            //full path to upload folder
            $uploadPath = WWW_ROOT . '/files/' . $uploadFolder;
            //check if there wasn't errors uploading file on serwer
            ...
            /**************Upload marker by rahul**************************/
             $marker = $this->request->data['Restaurant']['marker'];
            $uploadFolder = "restaurants";
            //full path to upload folder
            $uploadmarkerPath = WWW_ROOT . '/files/' . $uploadFolder;
            if ($marker['error'] == 0) {
                //image file name
                $markerName = $marker['name'];
                //check if file exists in upload folder
                if (file_exists($uploadmarkerPath . DS . $markerName)) {
                    //create full filename with timestamp
                    $markerName = date('His') . $markerName;
                }
                //create full path with image name
                $full_marker_path = $uploadmarkerPath . DS . $markerName;
                $this->request->data['Restaurant']['marker'] = $markerName;
                //upload image to upload folder
                move_uploaded_file($marker['tmp_name'], $full_marker_path);
            }else {

                $admin_edit = $this->Restaurant->find('first', array('conditions' => array('Restaurant.id' => $id)));
                $this->request->data['Restaurant']['marker'] = !empty($admin_edit['Restaurant']['marker']) ? $admin_edit['Restaurant']['marker'] : 'hotel.png';
            }
            
            
            $marker = $this->request->data['Restaurant']['webmarker'];
            $uploadFolder = "restaurants";
            //full path to upload folder
            $uploadmarkerPath = WWW_ROOT . '/files/' . $uploadFolder;
            if ($marker['error'] == 0) {
                ...
                //create full path with image name
                $full_marker_path = $uploadmarkerPath . DS . $markerName;
                $this->request->data['Restaurant']['webmarker'] = $markerName;
                //upload image to upload folder
                move_uploaded_file($marker['tmp_name'], $full_marker_path);
            }else {

                $admin_edit = $this->Restaurant->find('first', array('conditions' => array('Restaurant.id' => $id)));
                $this->request->data['Restaurant']['webmarker'] = !empty($admin_edit['Restaurant']['marker']) ? $admin_edit['Restaurant']['marker'] : 'hotel.png';
            }
            ...
            if ($this->Restaurant->save($this->request->data)) {
                ...
                $this->Session->setFlash('The Deal has been saved.');
                return $this->redirect(array('action' => 'index'));
            }
        } else {
            $options = array('conditions' => array('Restaurant.' . $this->Restaurant->primaryKey => $id));
            ...
            $this->set('restype', $restype);
            $this->loadModel('Cat');
            $restypes = $this->Cat->find("list");
            $this->set('restypes', $restypes);
        }
        $this->loadModel('User');
    ...
    public function api_restaurantslist() {
        ...
        if (!empty($redata)) {
            ...
            for ($i = 0; $i < $cnt; $i++) {
                if ($data[$i][0]['distance'] < $this->distance) {
                    $data[$i]['Restaurant']['logo'] = FULL_BASE_URL . $this->webroot . "files/restaurants/" . $data[$i]['Restaurant']['logo'];
                    $data[$i]['Restaurant']['marker'] = FULL_BASE_URL . $this->webroot . "files/restaurants/" . $data[$i]['Restaurant']['marker'];
                } else {
                    unset($data[$i]);
                }
            }
            ...
            foreach ($data as $restaurant) {
                if (!empty($restaurant['Cat']['image'])) {
                    $restaurant['Cat']['image'] = FULL_BASE_URL . $this->webroot . "img/" . $restaurant['Cat']['image'];
                }
                
                if (!empty($restaurant['Favrest'])) {
                    $fav_rest_user = array();
                    foreach ($restaurant['Favrest'] as $fav_rest) {
                        array_push($fav_rest_user, $fav_rest['user_id']);
                    }
                    if (isset($redata->user_id)) {
                        $set_as_fav = in_array($redata->user_id, $fav_rest_user);
                        if ($set_as_fav) {
                            $restaurant['Restaurant']['favrest'] = 1; 
                        } else {
                            $restaurant['Restaurant']['favrest'] = 0;
                        }
                    } else {
                        $restaurant['Restaurant']['favrest'] = 0;
                    }
...
//                }
	 ...
	 public function api_restaurantslistall() {
        ...
        if (!empty($redata)) {
            if ($lat == 0 || $long == 0) {
                $response['isSucess'] = "false";
                $response['msg'] = "There is no data availableee";
            } else {
                $data = $this->Restaurant->find('all',array('conditions'=>
				 array('Restaurant.catid'=>$alltype),
				 "fields" => array("get_distance_in_miles_between_geo_locations($lat,$long,Restaurant.latitude,Restaurant.longitude) as distance", "Restaurant.*,Cat.*,RestaurantsType.*"),
				 "ORDER BY" => 'DESC',
                ));
            }
            $cnt = count($data);
            ...
            if ($finaldata) {
                ...
                $response['isSuccess'] = "false";
                $response['msg'] = "There is no data available1"; 
            }
        } else {
            $response['isSuccess'] = "false";
    ...
    /****************************************/
 public function api_restaurantslistallwork() {
        configure::write('debug', 0);
        $this->layout = "ajax";
        $postdata = file_get_contents("php://input");
        $redata = json_decode($postdata);
          $lat = $redata->data->Restaurant->latitude;//=30.5389944;
        $long = $redata->data->Restaurant->longitude;//=75.9550329;
        $cuisineid_array=$redata->data->Restaurant->cuisineid;
        $response['pos']['lat'] = $lat;
        $response['pos']['lng'] = $long;
        if (!empty($redata)) {
            if ($lat == 0 || $long == 0) {
            ...
            $j = 0;
            $cuisines = array();
            if(isset($cuisineid_array)){
                foreach($data as $restaurant){
                    $unserialize = unserialize($restaurant['Restaurant']['typeid']);
                    if(!empty($unserialize) && is_array($unserialize)){
                        foreach($unserialize as $single_id){
                            if(in_array($single_id,$cuisineid_array)){
                                array_push($cuisines, $restaurant['Restaurant']['id']);
                            }
                        }
                    }
                }
            }
            $data = $this->Restaurant->find('all',array('conditions'=> array('Restaurant.id'=> $cuisines),
                "fields" => array("get_distance_in_miles_between_geo_locations($lat,$long,Restaurant.latitude,Restaurant.longitude) as distance", "Restaurant.*,Cat.*,RestaurantsType.*"),
				 "group"=>'Restaurant.id',
                                    "ORDER BY" => 'DESC',));
             foreach($data as $restaurant){    
                 if(!empty($restaurant['Favrest'])){    
                  foreach($restaurant['Favrest'] as $fvst){
                      if($fvst['user_id']==$redata->user_id){
                          $restaurant['Restaurant']['favrest']="1";
                          break;
                      }else {
                          $restaurant['Restaurant']['favrest']="0";
           ...
           $cnt = count($finaldata);
            for ($i = 0; $i < $cnt; $i++) {
                if ($finaldata[$i][0]['distance'] < $this->distance) {
                     $finaldata[$i]['Restaurant']['logo'] = FULL_BASE_URL . $this->webroot . "files/restaurants/" . $finaldata[$i]['Restaurant']['logo'];
                     $finaldata[$i]['Restaurant']['marker'] = FULL_BASE_URL . $this->webroot . "files/restaurants/" . $finaldata[$i]['Restaurant']['marker'];
                     $finaldata[$i]['Cat']['image'] = FULL_BASE_URL . $this->webroot . "img/" . $restaurant[$i]['Cat']['image'];
                } else {
                    unset($finaldata[$i]);
                }
            }
...
//        $resid=642;
		...
		$this->loadModel('Deal');					
		 if ($this->Deal->hasAny(array('Deal.uid' => $ursid,'Deal.rid' => $resid))){

//echo "<pre>";print_r($this->request->data['Deal']['rid']);
                        // $allval =  $this->Deal->updateAll(array('Deal.rid' => $resid)); 
//echo "rahul";print_r($allval);
			}else{
			$this->request->data['Deal']['uid'] = $ursid;
			$this->request->data['Deal']['rid'] = $resid;
	...
	public function api_deals(){
        ...
        if($ursid){
            ...
            $zdata = $this->Deal->find('all', array(
                'group'=>'Deal.rid',
                'conditions' => array('Deal.uid' => $ursid),'order'=>'Deal.id DESC')); 
            if ($zdata) {
                $response['isSuccess'] = "true";
                $response['data'] = $zdata;
            } else {
            ...
            $response['isSuccess'] = "false";
            $response['msg'] = "There is no user available";
        }
        //debug($finaldata);exit;
        echo json_encode($response);
        $this->render('ajax');
...
//        }
	...
	public function orderhistory() {
              ...
              $data = $this->Restaurant->find('all', array(
                ...
                'joins' => array(
                    array(
                        ...
                        //'order'=>array('favres.res_id'=>'Restaurant.id')
                    )
                ),
                   'order'=>array('favres.id' =>'DESC'),
                'fields' => array('favres.*', 'Restaurant.*')
            ));
          // print_r($data);
            $this->set('favlist',$data);
       ...
       $this->set('fav',$favIds);
       $this->loadModel('Cat');
       $catname = $this->Cat->find('all');
       foreach($catname as $ct){
           
          $catnames = $ct['Cat']['rest_type'];
       }
       $this->set('ctname',$catnames);
        }
		...
		$this->Order->updateAll(array('Order.dl_status' => 2), array('Order.id' => $id)); 
		 ...
		 echo "<script>window.location.assign('http://rajdeep.crystalbiltech.com/dropin/restaurants/orderhistory')</script>";
		}
	...
	public function res_review($id) {
			...
			/*if ($this->User->hasAny(array('Review.uid' => $this->Auth->user('id'),'Review.resid' => $id))) {
				...
				echo "<script>alert('Username already exist!!!')</script>";
				/*echo "<script>window.location.assign('http://rajdeep.crystalbiltech.com/dropin/')</script>";*/
 ...
 public function api_getalltype() {
    ...
    public function api_faq(){
         configure::write('debug', 0);    
           ...
           $data = $this->Staticpage->find('all',array('conditions' => array('Staticpage.position'=> 'faq','AND'=>array('Staticpage.status'=>1))));
