	...
	$this->newbaseurl;
       ...
       // print_r($res_type);exit;
        //$this->set('res_types',$res_type);
         ...
         $this->loadModel('User');
         $this->loadModel('RestaurantsType');
        foreach($res_type as $rid){
            
            $resCount[] = $this->Restaurant->find('all',array('conditions'=>array('Restaurant.catid'=>$rid['Cat']['id'])));
        }
        //print_r($resCount);exit;
         $this->set('restcount',$resCount);
       ...
       // print_r($hits);
        $this->set('fav',$resIDS);
        $cities = $this->Restaurant->find('all', array('conditions' => array('Restaurant.id' => $resIDS), 'group' => 'Restaurant.city'));
        $this->set('cities', $cities);
        if(isset($cate) && isset($location)){
        if($cate != 'all' && $location != 'all'){
        $this->Restaurant->recursive=1;
        $data = $this->Restaurant->find('all', array('conditions' => array('AND' => array('Restaurant.id' => $resIDS,'Restaurant.catid' => $cate, 'Restaurant.city' => $location,'not' => array('Restaurant.hits' => 0)))));    
        }elseif($cate == 'all' && $location != 'all'){ 
         $this->Restaurant->recursive=1;
        $data = $this->Restaurant->find('all', array('conditions' => array('AND' => array('Restaurant.id' => $resIDS, 'Restaurant.city' => $location,'not' => array('Restaurant.hits' => 0)))));  
        }
        elseif($cate != 'all' && $location == 'all'){
        $this->Restaurant->recursive=1;
        $data = $this->Restaurant->find('all', array('conditions' => array('AND' => array('Restaurant.id' => $resIDS, 'Restaurant.catid' => $cate,'not' => array('Restaurant.hits' => 0)))));  
        }else{
           $this->Restaurant->recursive=1;
             $data = $this->Restaurant->find('all', array( 
                //'fields' => array('MAX(Restaurant.hits) AS hits', '*'), 
                   'order by'=>array('MAX(Restaurant.modified) desc'),
                    'conditions' => array('not' => array('Restaurant.hits' => 0),
                      //'order'=>'Restaurant.created DESC'
            )));
            //'order' => array('Model.created', 'Model.field3 DESC'),
            
           // $data = $this->Restaurant->find('all', array('conditions' => array('Restaurant.id' => $resIDS),
          // ));
           ...
           $data = $this->Restaurant->find('all', array( 
                ...
                'order'=>array('Restaurant.hits desc'),
                'limit'=>8
            ));
           
           // $data = $this->Restaurant->find('all', array('conditions' => array('Restaurant.id' => $resIDS),'NOT'=>array('Restaurant.hits'=>array(0)),'order' => 'Restaurant.hits DESC'));
         
     // print_r($data);
      
       // print_r($popularDeal);exit;
       ...
       // print_r($favIds);exit;
       $this->set('favs',$favIds); 

    }
 
    $this->set('popularDeal',$data);
    //$url = json_decode(file_get_contents("http://ipinfo.io/".$_SERVER['REMOTE_ADDR']."/json"));
      ...
      function searchData(){
          ...
          $this->layout = 'ajax';
            ...
            //$url = json_decode(file_get_contents("https://www.googleapis.com/geolocation/v1/geolocate?key=AIzaSyAFBwip8aVt9LCuo3GxTzHwLiJT2jgh-uM"));
         // echo $url->loc;
